...
// @run-at document-end
// @copyright  2012+, Binux <root@binux.me> Tofuliang <tofuliang@gmail.com>
...
// Created on Fri 20 Jul 2012 11:43:22 AM CST
    ...
    TLE.exporter = {
        '复制链接': function (todown) {
            ...
            str += "</ul>";
            $("#TLE_text_pop").tpl("TLE_text_tpl", {
                'title': '复制选中的链接 &gt; <a href="' + "data:text/html;charset=utf-8," + encodeURIComponent(str) + '" target="_blank">在新窗口中打开</a>',
                'content': str
            }).show().pop({
                onHide: function () {
                    $(document.body).click();
        ...
        "YAAW": function (todown, rpc) {
            rpc = rpc || (function () {
                    rpcs = TLE.getConfig("TLE_aria2_jsonrpc");
                    return rpcs === "" ? "" : rpcs.split("|").shift();
                })();
            if (rpc) {
                show_tip("添加中...到YAAW(" + rpc + ")界面查看是否添加成功");
                var aria2 = new ARIA2(rpc);
                $.each(todown.tasklist, function (n, task) {
                    $.each(task.filelist, function (l, file) {
                        if (!file.downurl) return;
                        var filepath = TLE.safe_title(file.title);
    ...
    (function (TLE) {
        ...
        TLE.down = function (_this, _do) {
            ...
            if (info.d_tasktype == "0") { //bt task
                ...
                $.getJSON(INTERFACE_URL + "/fill_bt_list?tid=" + info.input + "&g_net=" + G_section + "&uid=" + G_USERID + "&callback=?", function (data) {
                    ...
                    if ('' !== $(_this).attr('data-rpc')) {
                        var rpc = $(_this).attr('data-rpc');
                        _do(todown, rpc);
                    } else {
                        _do(todown);
                    }
        ...
        TLE.add_YAAW_rpc = function (_this) {
            var i = $("input[name=TLE_aria2_jsonrpc\\[\\]]").length;
            $(_this).parent().parent().append('<li>Path: <input type="text" name="TLE_aria2_jsonrpc[]" id="TLE_aria2_jsonrpc' + (i + 1) + '" style="width: 350px" /></li>');
        };
        ...
        function init() {
            ...
            $("head").append('<style>'
                ...
                + '.TLE_getbtn {position: absolute; top:24px; left:0; border:1px #6FB2F3 solid; background:#fff; width:auto;-moz-border-radius:3px;-webkit-border-radius:3px;border-radius:3px;-moz-box-shadow:2px 2px 3px #ddd;-webkit-box-shadow:2px 2px 3px #ddd;}'
                + '.TLE_getbtn a {display:block; height:22px; line-height:22px; padding:0 18px;}'
            ...
            $("#setting_main_tpl").text($("#setting_main_tpl").text().replace(/(<\/div>\s+<div class="btnin">)/,
                ...
                + '<li><b class="json_rpc_title">Aria2 JSON-RPC Path</b><a href="#" class="btn_nol add_json_rpc" title="添加" onclick="TLE.add_YAAW_rpc(this)">添加</a></li>'
                + (function () {
                    var jsonrpcs = TLE.getConfig("TLE_aria2_jsonrpc").split("|");
                    var str = '';
                    $.each(jsonrpcs,function (i, p) {
                        str += '<li>Path: <input type="text" name="TLE_aria2_jsonrpc[]" id="TLE_aria2_jsonrpc' + (i + 1) + '" style="width: 350px" value="' + p + '"/></li>';
                    });
                    return str;
                })()
                + '</ul>'
            ...
            set_notice_submit = function (f) {
                ...
                var jsonrpc_path = (function () {
                    ...
                    $("input[name=TLE_aria2_jsonrpc\\[\\]]").each(function (index, html) {
                        var rpc = $.trim($(html).val());
                        if ('' !== rpc) jsonrpcs.push(rpc);
                    });
                    jsonrpcs = $.unique(jsonrpcs);
                    return jsonrpcs.join("|");
                })();
            ...
            function exporter_anchors(type) {
                ...
                $.each(TLE.exporter, function (n, f) {
                    if (n == 'YAAW' && enabled_exporter.indexOf(n) !== -1 ) {
                        useYAAW = true;
                        return;
                    }
                ...
                if (useYAAW) {
                    $.each(TLE.getConfig("TLE_aria2_jsonrpc").split("|"), function (n, f) {
                        str += ('<a href="#" title="YAAW(' + f + ')" onmouseover="this.className=\'sel_on\'" data-rpc="' + f + '" onmouseout="this.className=\'\'" onclick="' + type + '(this, TLE.exporter[\'YAAW\'])">YAAW(' + f + ')</a>');
                    });
                }
                return str;
            }
            ...
            $("div.TLE_get_btnbox").click(function (e) {
                e.stopPropagation();
            });
        }
    ...
    (function (TLE) {
        ...
        function init() {
            ...
            $("#TLE_setting_anchor").on("click", function () {
                ...
                $("#TLE_text_pop").tpl("TLE_text_tpl", {
                    title: "Thunder Lixian Exporter 设定",
                    content: content
                }).show().pop();
            ...
            function show_exporter_selector() {
                $("#TLE_text_pop").tpl("TLE_text_tpl", {
                    title: "您正在使用Thunder Lixian Exporter",
